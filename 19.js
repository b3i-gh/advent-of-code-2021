...
function create_beacon_thumbprints(A) {
  let beacons = [];
  A.forEach((scanner, scanner_idx) => {
  ...
  while (found_scanners.size < A.length - 1) {
    ...
    for (const [idx, match] of found_scanners) {
      for (const beacon of A[idx]) {
        ...
        if (!A[0].some((b) => b[0] == a[0] && b[1] == a[1] && b[2] == a[2]))
          A[0].push(a);
      }
    }
    ...
    beacons = create_beacon_thumbprints(A);
  }
